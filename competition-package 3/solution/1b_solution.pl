...
while(<STDIN>){
    if($_=~/([\w\s]+)\|\s?([\d\s]*)\|\s?(.*)/) {
        ...
        my $msg = $3;
        my $output = "";
#        print "DEBUG: $OP ::: $key_string ::: $msg\n";
        # process it here
        # get the value of the letter
        my @key = split(/\s/,$key_string);
        #print Dumper(@key);
        if ($OP =~ /ENCRYPT/) {
            my $index = 0;
            foreach my $char (split(//,$msg)) {
                ...
                if (exists $A_to_one{$char}) {
                    ...
                    my $encrypted_value = ($original_value + $key[$index % scalar(@key)])%26;
            ...
            foreach my $char (split(//,$msg)) {
                if (exists $A_to_one{$char}) {
                    ...
                    my $original_value = ($encrypted_value - $key[$index % scalar(@key)])%26;
